...
class ControllerPaymentSisow extends Controller {
	...
	public function _index($payment) {
		...
		else if ($payment == 'sisowklarna' || $payment == 'sisowklaacc' || $payment == 'sisowfocum'|| $payment == 'sisowcapayable'|| $payment == 'sisowbillink') {
			$data['text_description'] = $this->language->get('text_description');
			$data['text_phone'] = $order_info['telephone'];
			$data['text_klarnaid'] = $this->config->get($payment . '_klarnaid');
			if ($payment == 'sisowklarna' || $payment == 'sisowfocum' || $payment == 'sisowcapayable' || $payment == 'sisowbillink') {
		...
		} else if (file_exists(DIR_TEMPLATE . 'default/template/payment/'.$payment.'.tpl')) {
	...
	public function _redirectbank($payment) {
		...
		if($payment == 'sisowbillink' && !array_key_exists('bilterms', $this->request->post)){
			$json['error'] = 'U dient akkoord te gaan met de Billink voorwaarden.';
			$this->response->addHeader('Content-Type: application/json');
			$this->response->setOutput(json_encode($json));
			return;
		}
		...
		{		
			...
			*/
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status')) {
					...
					if ($result['code'] === 'shipping') {
						var_dump($total_data, $total_data['totals'], $total_data['taxes']);
                    }
			...
			if (($ex = $sisow->TransactionRequest($posts)) < 0) {
				...
				if($payment == 'sisowklarna' || $payment == 'sisowklarnaacc' || $payment == 'sisowfocum' || $payment == 'sisowafterpay' || $payment == 'sisowbillink')
